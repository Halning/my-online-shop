...
export class Migration20230925122745 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table "cart" ("id" uuid not null, "user_id" varchar(255) not null, "is_deleted" boolean not null, constraint "cart_pkey" primary key ("id"));');
    this.addSql('alter table "cart" add constraint "cart_user_id_unique" unique ("user_id");');

    this.addSql('create table "order" ("id" varchar(255) not null, "user_id" varchar(255) not null, "cart_id" uuid not null, "items" jsonb not null, "payment" jsonb not null, "delivery" jsonb not null, "comments" varchar(255) not null, "status" smallint not null, "total" int not null, constraint "order_pkey" primary key ("id"));');

    this.addSql('create table "cart_item" ("id" varchar(255) not null, "count" int not null, "product_id" varchar(255) not null, "cart_id" uuid not null, constraint "cart_item_pkey" primary key ("id"));');
